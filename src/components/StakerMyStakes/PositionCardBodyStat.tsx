        ...
        <div className={'f mb-1'}>
            <div className={'f mr-1 w-100'}>
                <CurrencyLogo size={'35px'}
                              currency={new Token(137, rewardToken?.id, 18, rewardToken?.symbol) as WrappedCurrency} />
                <div className={'ml-05'}>
                    <h3 className={'fs-075 mb-025'}>Reward</h3>
                    <div title={earned.toString()}>{`${formatReward(+earned)} ${
                        rewardToken.symbol
                    }`}</div>
                </div>
            </div>
            <div className={'f mr-1 w-100'}>
                <CurrencyLogo size={'35px'}
                              currency={new Token(137, bonusRewardToken?.id, 18, bonusRewardToken?.symbol) as WrappedCurrency} />
                <div className={'ml-05'}>
                    <h3 className={'fs-075 mb-025'}>Bonus reward</h3>
                    <div title={bonusEarned.toString()}>{`${formatReward(+bonusEarned)} ${
