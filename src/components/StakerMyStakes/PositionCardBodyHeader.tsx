...
export default function PositionCardBodyHeader({ el, farmingType, date, enteredInEternalFarming, eternalFarming }: PositionCardBodyHeaderProps) {
    ...
    useEffect(() => {
        farmingType === FarmingType.FINITE && console.log(el)
    }, [el])
    ...
    return (
        <div className={`flex-s-between b mb-1 fs-125 ${farmingType === FarmingType.ETERNAL ? 'farming-card-header ms_fd-c' : ''}`}>
            ...
            {
                el?.lockedToken && <div className={'f f-ac ml-2 mxs_ml-0 mxs_mv-1'}>
                    ...
                    <div className={'ml-05'}>
                        ...
                        <div>{`${formatAmountTokens(+formatUnits(BigNumber.from(el?.lockedToken.amount), el?.lockedToken.decimals))} ${el?.lockedToken.symbol}`}</div>
                    </div>
                </div>
            }
        </div>
