    ...
    return (
        <div className={"f f-jc f-ac"}>
            <NavLink className={"link f-ac"} to={`/info/pools/${address}`}>
                <DoubleCurrencyLogo
                    currency0={new Token(AlgebraConfig.CHAIN_PARAMS[chainId || 100].chainId, token0?.id, 18, token0.symbol) as WrappedCurrency}
                    currency1={new Token(AlgebraConfig.CHAIN_PARAMS[chainId || 100].chainId, token1?.id, 18, token1.symbol) as WrappedCurrency}
                    size={20}
                />
                <TYPE.label ml="8px">
            ...
            <a className={" hover-op trans-op"} href={`${AlgebraConfig.CHAIN_PARAMS[chainId || 100].blockExplorerURL}/address/${address}`} rel="noopener noreferrer" target="_blank">
