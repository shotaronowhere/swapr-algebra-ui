...
export default function Chart({ feeData: { data, previousData }, span, type, dimensions, isMobile, tokens: { token0, token1 }, token }: ChartInterface) {
    ...
    const Focus = create("svg:circle").style("fill", "white").attr("stroke", "var(--primary)").attr("stroke-width", "2").attr("r", 5.5).style("opacity", 1).style("display", "none");
            ...
            .map((el, i) => {
                ...
                const hoverHandle = function (e: any) {
                    ...
                    InfoRectGroup.attr(
                        "transform",
                        //@ts-ignore
                        `translate(${isOverflowing ? Number(xTranslate) - 150 - 16 : Number(xTranslate) - (type === ChartType.PRICE ? 95 : 80)},${y(_chartData[i]?.value) - 68})`
                    );
