...
export default function RealStakerPage({}) {
  ...
  const { stakerHandler, stakerClaimHandler, stakerUnstakeHandler, frozenStakedHandler, frozenStaked } =
  ...
  useEffect(() => {
    ...
    if (percentForSlider === 0) {
      ...
      setAmountValue(
        formatEther(parseUnits(numBalance?.toSignificant(4), 18).div(BigNumber.from('100')).mul(percentForSlider))
      )
    }
  }, [percentForSlider])
    ...
    <RealStakerPageWrapper>
      ...
      <EarnedStakedWrapper width={'765px'}>
        <ResBlocksTitle>
            ...
            {frozenStaked?.length !== 0 && (
              ...
              >
                {!allFreeze ? (
                  ...
                  `${(+formatEther(allFreeze || BigNumber.from('0'))).toFixed(2) < 0.01 ? '<' : ''} ${(+formatEther(
                    allFreeze
                  )).toFixed(2)}`
                )}{' '}
                ALGB Frozen {showFrozen ? <ArrowUp size={'16px'} /> : <ArrowDown size={'16px'} />}
              </FrozenDropDown>
            )}
          ...
          {showFrozen &&
          frozenStaked?.length !== 0 &&
          frozenStaked?.some((el) => +Math.floor(el.timestamp * 1000) > now()) ? (
            <Frozen data={frozenStaked} earnedFreeze={earnedFreezeArr} now={now} />
          ) : null}
        </ResBlocksTitle>
        <ResBlocksWrapper>
          <RealStakerResBlocks
