...
export function Aftermath({ done }: IAftermath) {
    ...
    return <div className='f c f-ac f-jc w-100' style={{ height: 'calc(414px + 4rem)' }}>
        {
            isConfirmed ? <>
                <div> <CheckCircle size={'36px'} stroke={"var(--green)"} /></div>
                <div className='mt-1 fs-125'>Liquidity added!</div>
                <Link to={'/pool?newest=true'} className='go-to-pools mt-2'>Go to pools</Link>
            </>
                :
                <>
                    <div><Loader size={'36px'} stroke={'white'} /></div>
