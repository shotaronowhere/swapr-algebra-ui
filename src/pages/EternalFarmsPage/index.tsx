...
export default function EternalFarmsPage({
    ...
    <PageWrapper>
        {
            refreshing ? <div>Loading</div> : !data || data.length === 0 ? <div>No farms</div> : !refreshing && data.length != 0 ?  
            <EternalFarmsList>
                {
                    data.map((farm, i) => 
                    <EternalFarmsItem key={i}>
                        ...
                        <EternalFarmsBody>
                            <PoolWrapper>
                                <PoolWrapperLogos>
                                    <CurrencyLogo currency={{address: farm.token0.id, symbol: farm.token0.symbol}} size='35px'></CurrencyLogo> 
                                    <CurrencyLogo currency={{address: farm.token1.id, symbol: farm.token1.symbol}} size={'35px'}></CurrencyLogo>
                                </PoolWrapperLogos>
                                <div>
                            ...
                            <span>XXXX</span>
                            <span>
                                <FarmButton>Farm</FarmButton>
                            </span>
                        </EternalFarmsBody>
                    </EternalFarmsItem>)
                }
        </EternalFarmsList> : null
        }
    </PageWrapper>
    </>
}