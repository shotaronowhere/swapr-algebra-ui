...
export const FACTORY_ADDRESS = V2_FACTORY_ADDRESSES[AlgebraConfig.CHAIN_PARAMS[100].chainId];
...
export const computePairAddress = ({
    ...
    return getCreate2Address(factoryAddress, keccak256(["bytes"], [pack(["address", "address"], [token0.address, token1.address])]), hash);
};
...
export class Pair {
    ...
    public constructor(currencyAmountA: CurrencyAmount<Token>, tokenAmountB: CurrencyAmount<Token>, sushi?: boolean) {
        ...
        this.liquidityToken = new Token(tokenAmounts[0].currency.chainId, Pair.getAddress(tokenAmounts[0].currency, tokenAmounts[1].currency, sushi), 18, "UNI-V2", "Uniswap V2");
    ...
    public getLiquidityValue(token: Token, totalSupply: CurrencyAmount<Token>, liquidity: CurrencyAmount<Token>, feeOn = false, kLast?: BigintIsh): CurrencyAmount<Token> {
        invariant(this.involvesToken(token), "TOKEN");
