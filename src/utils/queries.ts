...
export function useDeltaTimestamps(): [number, number, number] {
    ...
    const t1 = utcCurrentTime.subtract(1, 'day').startOf('day').unix()
    const t2 = utcCurrentTime.subtract(2, 'day').startOf('day').unix()
    const tWeek = utcCurrentTime.subtract(1, 'week').startOf('day').unix()
    return [t1, t2, tWeek]
}
